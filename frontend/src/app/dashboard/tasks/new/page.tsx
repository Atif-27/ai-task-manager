...
export default function TaskForm() {
    ...
    const toggleUserSelection = (userId: string) => {
        setFormData((prev) => {
            ...
            return {
                ...prev,
                assignedTo: isSelected ? prev.assignedTo.filter((id) => id !== userId) : [...prev.assignedTo, userId],
            };
        });
    };
                            ...
                            <div className="grid gap-3">
                                ...
                                <Select value={formData.status} onValueChange={(value) => handleSelectChange("status", value)}>
                                    ...
                                    <SelectContent>
                                        ...
                                        <SelectItem value={StatusType.COMPLETED}>Completed</SelectItem>
                                    </SelectContent>
                                </Select>
                            </div>
                            ...
                            <Popover>
                                <PopoverTrigger asChild>
                                    <Button variant="outline" className="justify-between">
                                        {formData.assignedTo.length > 0 ? `${formData.assignedTo.length} user(s) selected` : "Select users"}
                                        <ChevronsUpDown className="ml-2 h-4 w-4 opacity-50" />
                                    </Button>
                                </PopoverTrigger>
                                <PopoverContent className="w-full p-0">
                                    {users.map((user) => (
                                        <div key={user.id} className="flex items-center p-2 cursor-pointer" onClick={() => toggleUserSelection(user.id)}>
